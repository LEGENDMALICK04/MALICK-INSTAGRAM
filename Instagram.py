...
def getkey():
    ...
    try:
        ...
        if yt in ["Y","y"]:
            os.system("xdg-open https://wa.me/+918266803073?text=Hai+bg+rif,+aku+mau+beli+scriptnya+tapi+yang+versi+premium.+Ini+lisensinya:%20"+crot+"+konfirmasi+nama+pembeli:%20"+namamu)
            open(".key.txt","w").write(crot+"\n"+namamu)
            exit()
        else:
            exit("\033[0m•> Telah keluar program")
    try:
        confirmkey = requests.get("https://github.com/Billa303/APROVAL2/blob/main/Updatebookmark.txt").json()
    except requests.exceptions.ConnectionError:
